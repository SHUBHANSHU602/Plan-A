...
def make_service(repository, now, records=None) -> AlertService:
    return AlertService(
...
async def assert_low_risk_does_not_touch_alert_storage() -> None:
    ...
    service = make_service(repository, datetime(2026, 9, 12, tzinfo=UTC))
...
async def assert_repeated_and_escalated_alert_actions() -> None:
    ...
    service = make_service(repository, now, records)
    cell_id = uuid4()
...
async def assert_alert_refreshes_after_cooldown() -> None:
    ...
    repository = StubAlertRepository()
    first = await make_service(repository, first_seen).evaluate(
        make_snapshot("HIGH", 0.70),
        "A17",
    )

    refreshed = await make_service(repository, first_seen + timedelta(minutes=31)).evaluate(
